...
def populate_tree(tree, data):
    for item in data:
        ...
        date_node = tree.insert(location_node, "end", text="Date")
        for key, value in item["Date"].items():
            tree.insert(date_node, "end", text=f"{key}: {value}")
...
data = {}
with open("D:/nuclear_explosions.json", "r") as file:
    data = json.load(file)
